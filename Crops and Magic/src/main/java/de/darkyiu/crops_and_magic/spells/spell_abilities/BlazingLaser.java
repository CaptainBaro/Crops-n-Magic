...
public class BlazingLaser implements SpellAbility {
    ...
    public void onRightCLick(Player player, ItemStack itemStack) {
        if (player.getWorld().getName().contains("nether")){
            BasicUtility.shootLaser(player,player.getEyeLocation(), 30, 2.5f, 12, Particle.FLAME, Sound.ITEM_BUCKET_FILL_LAVA,true, false, false);
        }else {
            BasicUtility.shootLaser(player,player.getEyeLocation(), 30, 2.5f, 10, Particle.FLAME, Sound.ITEM_BUCKET_FILL_LAVA, true, false, false);
        }
    }
